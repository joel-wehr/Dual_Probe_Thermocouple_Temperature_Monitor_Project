    ...
    <body>
        <div class='container'>
            <div class='well' style='max-width: 400px; margin: 0 auto 10px; text-align:center;'>
                ...
                <h1>Probe 1: ";
const html2 = @"&degF</h1><h1>Probe 2: ";
const html3 = @"&degF</h1>
            </div>
        </div>
    </body>
...
APIKEY <- "Your Xively API Key goes here.";
...
device.on("Xively", function(v) {
    ...
    //channel3.Set(v.probe1reftemp, "");
    //channel4 <- Xively.Channel("Food_Probe_Temp_Reference")
    //channel4.Set(v.probe2reftemp, "");
    feed <- Xively.Feed("FEED ID GOES HERE", [channel1, channel2]);
    client.Put(feed);
});
